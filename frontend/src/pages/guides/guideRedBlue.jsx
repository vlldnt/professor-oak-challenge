...
const GuideRedBlue = () => {
  ...
  const [caught, setCaught] = useState(() => {
    const saved =
      typeof window !== "undefined"
        ? window.localStorage.getItem("oak-caught")
        : null;
  ...
  const EvolutionCard = ({ startId }) => {
    ...
    const allChainIds = chain
      .map((step) => step.from.id)
      .concat(
        chain.length > 0 && chain[chain.length - 1].to
          ? chain[chain.length - 1].to.id
          : []
      )
      .filter(Boolean);
      ...
      if (caught.includes(id)) {
        ...
        if (isFullChainCaught) {
          ...
          if (isTo) {
            ...
            // Dernier from
            if (idx === chain.length - 1 && !chain[chain.length - 1].to)
              toggleCaught(id);
          }
        } else {
          ...
          if (isTo) {
            ...
            if (chain[idx].to && !caught.includes(chain[idx].to.id))
              toggleCaught(id);
            // Si la suivante (from) n'est pas validée (pour les étapes intermédiaires)
            else if (chain[idx + 1] && !caught.includes(chain[idx + 1].from.id))
              toggleCaught(id);
    ...
    return (
      <div
        className={`flex items-center bg-white rounded-lg p-3 mb-2 overflow-x-auto w-fit ml-0 sm:p-2 sm:mb-1 max-w-full sm:max-w-[98vw] relative ${
          shouldFade ? "opacity-30 pointer-events-none" : ""
        }`}
      >
        {chain.map((step, idx) => {
          // Pour chaque étape, on ne peut cliquer que sur le premier ou si le précédent est attrapé
          const canClickFrom = shouldBlock
            ? false
            : idx === 0 ||
              (caught.includes(chain[idx - 1].from.id) &&
                (idx === 1 || caught.includes(chain[idx - 1].to?.id)));
                  ...
                  <div className="flex flex-col items-center mx-2 pr-2 pf-2 sm:mx-1 sm:pr-1">
                    ...
                    <span className="text-s text-green-600 font-bold mt-1 sm:text-xs sm:mt-0.5">
                      {step.evolution.level
                        ? i18n.language === "fr"
                          ? ` Niv. ${step.evolution.level}`
                          : ` Lv. ${step.evolution.level}`
                        : step.evolution.stone
                        ? step.evolution.stone[i18n.language] ||
    ...
    <div className="flex flex-row-reverse">
      ...
      <div className="hidden lg:block w-[45%] flex-shrink-0 p-6">
        <div className="sticky top-8">
          ...
          {/* Tableau des pokémonq manquants */}
          <h4 className={`text-sm font-bold text-center text-blue-700" mb-1`}>
        ...
        <section className="mb-8">
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 space-y-3">
            <p className="text-purple-800 text-sm">
              <strong>⏰ {t("timeInvestmentTitle")}</strong>{" "}
              {t("timeInvestment")}
            </p>
            <p className="text-purple-800 text-sm">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 mt-2 z-50 hidden group-hover:flex transition-all duration-200">
                  ...
                  >
                    <img
                      src={palletTownMap}
                      alt="Pallet Town Map"
                      className="rounded mr-2 h-auto"
                    />
                    <img
                      src={palletTown}
                      alt="Pallet Town"
                      className="rounded max-w-[220px] w-auto h-auto"
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="absolute left-1/2 -translate-x-1/2 mt-2 z-50 hidden group-hover:flex transition-all duration-200">
                  ...
                  >
                    ...
                    <img
                      src={route22}
                      alt="Route 22"
                      className="rounded max-w-[420px] w-auto h-[auto]"
                    />
                  </div>
                </div>
              </div>
            ...
            <h2 className="flex flex-row items-center text-2xl font-bold text-gray-900 border-b-2 border-gray-200 pb-2 mb-0">
              ...
              <div className="relative group ">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 mt-2 z-50 hidden group-hover:flex transition-all duration-200">
                  ...
                  >
                    <img
                      src={viridianForestMap}
                      alt="Viridian Forest Map"
                      className="rounded mr-2 h-auto"
                    />
                    <img
                      src={viridianForest}
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-lg font-bold">
                {t("caught")} {caught.length} / 21
              </p>
              <p className="text-sm opacity-90">
                {t("remaining")} {21 - caught.length}
              </p>
            </div>
